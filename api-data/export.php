            ...
            <?php foreach ($slides as $index => $slide): ?>
                ...
                <?php elseif ($slide['type'] === 'features'): ?>
                    ...
                    <?php elseif ($slide['type'] === 'location'): ?>
                        ...
                        $lat = $slide['location_lat'] ?? 55.751574;
                        $lng = $slide['location_lng'] ?? 37.573856;
                        $address = $slide['location_address'] ?? '';
                        $metroStations = $slide['metro_stations'] ?? [];
                        
                        // Генерируем имя файла на основе координат для кеширования
                        $mapCacheDir = __DIR__ . '/../cache/maps/';
                        if (!is_dir($mapCacheDir)) {
                            @mkdir($mapCacheDir, 0755, true);
                        }
                        
                        $mapFilename = 'map_' . md5($lat . '_' . $lng . '_v1_650x450') . '.png';
                        $mapPath = $mapCacheDir . $mapFilename;
                        
                        // Используем максимально допустимый размер: 650x450
                        $mapSize = '650,450';
                        
                        // Пробуем скачать карту если её нет
                        $mapDownloaded = false;
                        if (!file_exists($mapPath) || filesize($mapPath) == 0) {
                            $mapDownloaded = $yandexMaps->downloadStaticMap($lat, $lng, $mapPath, $mapSize, 15);
                        } else {
                            $mapDownloaded = true;
                        }
                        
                        // ВАЖНО: Для PDF конвертируем в base64 или используем прямой URL
                        if ($mapDownloaded && file_exists($mapPath) && filesize($mapPath) > 0) {
                            // Встраиваем изображение как base64 для PDF
                            $imageData = file_get_contents($mapPath);
                            $base64 = base64_encode($imageData);
                            $mapUrl = 'data:image/png;base64,' . $base64;
                        } else {
                            // Fallback - прямой URL к Яндекс Static API
                            $mapUrl = $yandexMaps->getStaticMapUrl($lat, $lng, $mapSize, 15);
                        }
                        ?>
                        <div class="booklet-page page-cover js-preview-page">
                            <div class="booklet-page__inner">
                                <div class="booklet-content booklet-map">
                                    <div class="booklet-map__wrap">
                                        <div class="booklet-map__title"><?php echo !empty($slide['title']) ? htmlspecialchars($slide['title']) : 'МЕСТОПОЛОЖЕНИЕ'; ?></div>
                                        <div class="booklet-map__img" style="width: 100%; overflow: hidden; background: #f0f0f0; position: relative;">
                                            <img 
                                                src="<?php echo htmlspecialchars($mapUrl); ?>" 
                                                alt="Карта местоположения" 
                                                style="width: 100%; height: 100%; object-fit: cover; display: block;"
                                                onerror="console.error('Map load error'); this.onerror=null; this.style.display='none'; this.parentElement.innerHTML='<div style=\'display:flex;align-items:center;justify-content:center;height:100%;color:#999;font-size:14px;flex-direction:column;padding:20px;\'><i class=\'fas fa-map-marked-alt\' style=\'font-size:48px;margin-bottom:10px;opacity:0.3;\'></i><div>Карта временно недоступна</div></div>';"
                                            />
                                        </div>
                                        <div class="booklet-map__content">
                                            ...
                                            <div class="booklet-map__info">
                                                <div class="booklet-map__name">
                                                    <?php echo !empty($slide['location_name']) ? htmlspecialchars($slide['location_name']) : ''; ?>
                                                </div>
                                                <div class="booklet-map__text" style="margin-bottom: 8px;">
                                                    <?php echo !empty($slide['location_address']) ? htmlspecialchars($slide['location_address']) : ''; ?>
                                                </div>
                                                
                                                <?php if (!empty($metroStations) && is_array($metroStations)): ?>
                                                    <div class="booklet-map__metro" style="margin-top: 12px; padding: 10px 0; border-top: 1px solid rgba(255,255,255,0.2);">
                                                        ...
                                                        <div style="font-size: 12px; line-height: 1.6; opacity: 0.95;">
                                                            <?php 
                                                            $displayStations = array_slice($metroStations, 0, 3);
                                                            foreach ($displayStations as $idx => $station): 
                                                            ?>
                                                                <div style="margin-bottom: <?php echo $idx < count($displayStations) - 1 ? '6px' : '0'; ?>; padding-left: 4px;">
                                                                    <div style="font-weight: 500; margin-bottom: 2px;">
                                                                        <?php echo htmlspecialchars($station['name']); ?>
                                                                    </div>
                                                                    <div style="opacity: 0.85; font-size: 11px; display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                                                                        <span style="display: inline-flex; align-items: center; gap: 4px;">
                                                                            ...
                                                                            <?php echo htmlspecialchars($station['walk_time_text']); ?>
                                                                        </span>
                                                                        ...
                                                                        <span style="display: inline-flex; align-items: center; gap: 4px;">
                                                                            ...
                                                                            <?php echo htmlspecialchars($station['drive_time_text']); ?>
                                                                        </span>
                                                                        <span style="opacity: 0.5;">•</span>
                                                                        <span><?php echo htmlspecialchars($station['distance_text']); ?></span>
                                                                    </div>
                                                                </div>
                                                            <?php endforeach; ?>
                                                        </div>
                        ...
                        <script>
                            // Инициализация карты для слайда <?php echo $index; ?>

                            (function() {
                                const mapData = {
                                    containerId: 'map-<?php echo $index; ?>',
                                    lat: <?php echo $lat; ?>,
                                    lng: <?php echo $lng; ?>,
                                ...
                                window.locationMaps.push(mapData);
                            })();
                        </script>
...
<!-- Подключение Яндекс.Карт для режима просмотра -->
<script src="https://api-maps.yandex.ru/2.1/?apikey=0d3f02e3-3a2a-426d-a3a1-9952dcb199b9&lang=ru_RU"></script>
<script>
    // Инициализация всех карт на странице
    if (window.locationMaps && window.locationMaps.length > 0) {
        ymaps.ready(function() {
            window.locationMaps.forEach(function(mapData) {
                const container = document.getElementById(mapData.containerId);
                if (!container) return;
                
                try {
                    const map = new ymaps.Map(mapData.containerId, {
                    ...
                    // Добавляем красный пин
                    const placemark = new ymaps.Placemark([mapData.lat, mapData.lng], {
                    ...
                    // Отключаем взаимодействие для режима просмотра
                    map.behaviors.disable(['drag', 'scrollZoom', 'dblClickZoom', 'multiTouch']);
                    
                    console.log('Карта инициализирована:', mapData.containerId);
                } catch (error) {
                    console.error('Ошибка инициализации карты:', error);
                    container.innerHTML = '<div style="display: flex; align-items: center; justify-content: center; height: 100%; background: #f0f0f0; color: #999;">Карта недоступна</div>';
                }
            });
        });
    }
