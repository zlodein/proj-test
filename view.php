<?php
// view.php - Публичный просмотр презентаций
...
require_once __DIR__ . '/includes/currency.php'; // Добавляем подключение валют
...
session_start(); // Начинаем сессию для проверки авторизации

// Получаем хэш из URL
$hash = $_GET['hash'] ?? '';
$path = $_SERVER['REQUEST_URI'] ?? '';

// Если запрос пришел по пути /view/{hash}
if (preg_match('/^\/view\/([a-f0-9]{32})$/', $path, $matches)) {
    $hash = $matches[1];
}

if (empty($hash)) {
    die('Публичная ссылка не указана или неверна');
}
...
if (!$presentation || $presentation['is_public'] != 1) {
    die('Публичная презентация не найдена или доступ закрыт');
}

// Проверяем, существует ли класс CurrencyConverter
if (!class_exists('CurrencyConverter')) {
    // Если класс не найден, создаем простую альтернативу
    class CurrencyConverter {
        ...
        public static function getRates() {
            // Возвращаем статические курсы валют (в реальном проекте брать из БД или API)
            return [
                'RUB' => 1,
                'USD' => 0.011,  // 1 RUB = 0.011 USD
                'EUR' => 0.0092, // 1 RUB = 0.0092 EUR
                'CNY' => 0.078,  // 1 RUB = 0.078 CNY
                'KZT' => 5.2     // 1 RUB = 5.2 KZT
            ];
        }
    }
}
...
// Для публичного просмотра всегда показываем все валюты
...
<head>
    ...
    <title><?php echo $title; ?> - Публичная презентация</title>
    ...
    <style>
        ...
        <?php 
        // Определяем, может ли владелец печатать (проверяем по тарифу владельца)
...
<body <?php if (!$canPrint) echo 'class="print-disabled"'; ?>>
    ...
    <div class="public-header">
        ...
        <div class="public-badge">
            <i class="fas fa-globe"></i> Публичная презентация
        </div>
    </div>
    ...
    <div class="preview-content">
                                            ...
                                            <div class="all-currencies">
                                                <?php foreach ($rates as $currCode => $rate): ?>
                                                    ...
                                                    <div class="currency-item">
                                                        ...
                                                        <span class="currency-name"><?php echo $currNames[$currCode] ?? $currCode; ?></span>
        ...
        <div class="print-disabled">
            <i class="fas fa-ban"></i>
            <h3>Экспорт в PDF недоступен</h3>
            <?php 
            // Проверяем, является ли текущий пользователь владельцем
            $isOwner = false;
            $userId = getCurrentUserId();
        ...
        <?php else: ?>
        <div id="pdf-controls" style="text-align: center; margin: 30px 0;">
    ...
    <div class="footer-info">
        <p>Эта презентация создана с помощью <a href="https://presentation-realty.ru" target="_blank">Presentation Realty</a> - онлайн-конструктора презентаций для риелторов.</p>
        <p>Для создания собственных профессиональных презентаций <a href="/auth/register.php">зарегистрируйтесь бесплатно</a>.</p>
    </div>
    ...
    function blockPrint() {
        ...
        document.addEventListener('keydown', function(e) {
            ...
            // Ctrl+S, Cmd+S (сохранение страницы)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const printBtns = document.querySelectorAll('button[onclick*="print"], button[onclick*="Print"]');
        ...
        // Блокируем drag and drop для изображений
        ...
        // Блокируем копирование содержимого
    ...
    function showPrintBlock() {
        ...
        if (overlay) {
            ...
            setTimeout(hidePrintBlock, 10000);
        }
    }
    ...
    // Дополнительная защита - блокируем если пользователь пытается обойти
    window.addEventListener('load', function() {
        // Проверяем, не открыта ли страница в iframe
        if (window.self !== window.top) {
            document.body.innerHTML = '<div style="text-align:center;padding:50px;color:#e74c3c;"><h2>Доступ запрещен</h2><p>Публичная презентация не может быть встроена в iframe.</p></div>';
        }
    });
    <?php else: ?>
    function printToPDF() {
        const originalTitle = document.title;
        document.title = "Презентация_<?php echo $presentation['id']; ?>_<?php echo date('Y-m-d'); ?>";
        window.print();
        setTimeout(() => {
