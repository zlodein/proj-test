...
class User {
    ...
    public function create($data) {
        $stmt = $this->db->prepare("
            INSERT INTO users (email, password, name, last_name, created_at, role_id, tariff_id, is_active) 
            VALUES (:email, :password, :name, :last_name, NOW(), :role_id, :tariff_id, 1)
        ");
        ...
        $stmt->execute([
            ...
            ':name' => $data['name'],
            ':last_name' => $data['last_name'] ?? null, // ДОБАВЛЕНО
     ...
     * ОБНОВЛЕНО: добавлена поддержка last_name
     */
    public function updateProfile($userId, $data) {
        $allowedFields = ['name', 'last_name', 'password']; // ДОБАВЛЕНО last_name
        $updates = [];
        $params = [];
        
        foreach ($data as $field => $value) {
            if (in_array($field, $allowedFields)) {
                // Для пароля проверяем, что он не пустой
                if ($field === 'password') {
                    if (!empty($value)) {
                        $updates[] = "`password` = ?";
                        $params[] = $value;
                    ...
                    // Для name и last_name разрешаем любые значения (включая пустую строку)
                    $updates[] = "`$field` = ?";
                    $params[] = $value ?? ''; // Если NULL, заменяем на пустую строку
                }
            }
        }
