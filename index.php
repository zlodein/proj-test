...
// Получаем полные данные пользователя включая user_img и last_name
...
<body>
            ...
            <div class="user-menu">
                <div class="user-info" id="userProfileBtn" style="cursor: pointer;">
                    <div class="user-avatar">
                        <?php echo renderUserAvatar($user, 40, 'user-avatar-small'); ?>
                    </div>
                    <div class="user-details">
                        <span class="user-name"><?php echo escape(getUserFullName($user)); ?></span>
        ...
        <div class="profile-modal-content">
            ...
            <div class="profile-avatar-section">
                <div style="margin-bottom: 15px;">
                    <?php echo renderUserAvatar($user, 120, 'user-avatar-large'); ?>
                </div>
                <h3><?php echo escape(getUserFullName($user)); ?></h3>
                <p><?php echo escape($user['email']); ?></p>
                <?php if (!empty($user['user_img']) && strpos($user['user_img'], 'http') === 0): ?>
                    <p class="oauth-source">
                        <i class="fas fa-link"></i> Аватар из 
                        <?php 
                            if (strpos($user['user_img'], 'yandex') !== false) echo 'Яндекс';
                            elseif (strpos($user['user_img'], 'vk.com') !== false) echo 'ВКонтакте';
                            elseif (strpos($user['user_img'], 'mail.ru') !== false) echo 'Mail.ru';
                            else echo 'OAuth';
                        ?>
                    </p>
                <?php endif; ?>
    ...
    <script>
        ...
        async function selectTariff(tariffId) {
            try {
                ...
                if (data.success) {
                    ...
                    showNotification(data.error || 'Ошибка изменения тарифа', 'error');
                }
            } catch (error) {
                console.error('Error:', error);
                showNotification('Ошибка соединения с сервером', 'error');
        ...
        function showNotification(message, type = 'info') {
            ...
            notification.style.cssText = 'position: fixed; top: 20px; right: 20px; z-index: 10000; min-width: 300px; animation: slideInRight 0.3s;';
